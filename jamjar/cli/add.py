...
class Add:
    ...
    def add_playlist(self, playlist_identifier):
        """
        Save the playlist and its tracks to the database.
        ...
        try:
            ...
            for track_item in tracks_data["items"]:
                ...
                print(f"Added track '{track_name}' by '{track_artist}'.")

            print(f"Playlist '{name}' added with tracks successfully.")
        except Exception as e:
            raise RuntimeError(f"Failed to add playlist: {e}")
...
def add(playlist):
    ...
    add_manager = Add(db, spotify_api)
